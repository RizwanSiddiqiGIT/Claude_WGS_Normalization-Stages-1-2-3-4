...
- The 2026-06-04 Stage 1 uncompressed FASTQ run reached fastp successfully but generated temp trimmed outputs failed validation before promotion. `gzip -t` failed on temp R1 with `invalid compressed data--crc error`, and BBTools `reformat.sh qin=33 verifypaired=t` then found a real malformed trimmed record: read `51751832` had 145 bases but only 22 quality characters. Do not promote or use `/home/rayzw/DNA-Linux/hg38/tmp/stage1_20260604_223304/R1_trimmed.fastq.gz` or `R2_trimmed.fastq.gz`. Treat this as a failed Stage 1 output, not merely a gzip-tool false positive.
- A new Stage 1 fork named `Stage 1 MGI qin33` exists in `stage1_mgi_qin33_preprocess.sh`. It keeps the original Stage 1 script intact but uses explicit MGI adapter sequences, poly-G trimming, `--qualified_quality_phred 15`, `--length_required 36`, and a streaming paired qin33 validator before promotion. Launch the DNA-Linux uncompressed real-data version with `run_stage1_mgi_qin33_dna_linux_uncompressed_foreground.sh`; its outputs are isolated under `/home/rayzw/DNA-Linux/hg38/fastq_mgi_qin33` and `/home/rayzw/DNA-Linux/hg38/qc_mgi_qin33`.
- `launch_stage1_mgi_qin33_smoke_test.sh` passed on 2026-06-05. The smoke test generated 100 paired synthetic MGI-style reads, fastp trimmed the explicit MGI adapters from all 200 reads, gzip validation passed, the streaming qin33 paired validator reported `PAIRS=100` and `BAD_PAIRS=0`, and final FastQC/MultiQC completed. MultiQC printed missing bundled JS asset warnings but still completed and wrote its report.
