...
FASTQ_OUT_DIR="${BASE_DIR_DNA_LINUX}/fastq_mgi_qin33"
QC_OUT_DIR="${BASE_DIR_DNA_LINUX}/qc_mgi_qin33"
TMP_OUT_DIR="${BASE_DIR_DNA_LINUX}/tmp"

mkdir -p "${LOGS_DIR_DNA_LINUX}" "${FASTQ_OUT_DIR}" "${QC_OUT_DIR}" "${TMP_OUT_DIR}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
LOG="${LOGS_DIR_DNA_LINUX}/stage1_mgi_qin33_uncompressed_${RUN_ID}.log"
LATEST_LOG="${LOGS_DIR_DNA_LINUX}/stage1_mgi_qin33_latest.log"
PID_FILE="${LOGS_DIR_DNA_LINUX}/stage1_mgi_qin33.pid"

{
  echo "$$" > "${PID_FILE}"
  ln -sfn "${LOG}" "${LATEST_LOG}"
  echo "======================================================================"
  echo " STAGE 1 MGI QIN33 DNA-LINUX UNCOMPRESSED FASTQ LAUNCHER"
  echo " Launcher PID: $$"
  echo " Started: $(date)"
  echo " Log: ${LOG}"
  echo " Raw R1: ${RAW_R1}"
  ...
  test -r "${RAW_R1}"
  test -r "${RAW_R2}"
  if ! pgrep -f "progress_tracker.py.*stage1_mgi_qin33" >/dev/null 2>&1; then
    LOG="${LOG}" \
    LATEST_LOG="${LATEST_LOG}" \
    ./run_stage1_mgi_qin33_progress_tracker_foreground.sh &
  ...
  echo "======================================================================"
  echo " STAGE 1 MGI QIN33 DNA-LINUX UNCOMPRESSED FASTQ FINISHED"
  echo " Finished: $(date)"
  echo "======================================================================"
} >> "${LOG}" 2>&1
